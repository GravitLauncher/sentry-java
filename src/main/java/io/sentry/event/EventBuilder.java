...
public class EventBuilder {
	 ...
	 * <p>
	 * The {@code InetAddress.getLocalHost().getCanonicalHostName()} call can be
	 * quite expensive and could be called for the creation of each {@link Event}.
	 * This system will prevent unnecessary costs by keeping track of the hostname
	 * for a period defined during the construction.<br>
	 * For performance purposes, the operation of retrieving the hostname will
	 * automatically fail after a period of time defined by
	 * {@link #GET_HOSTNAME_TIMEOUT} without result.
	 */
	private static final class HostnameCache {
		 ...
		 * <p>
		 * Gets the value from the cache if possible otherwise calls
		 * {@link #updateCache()}.
		 *
		 * @return the hostname of the current machine.
		 */
		public String getHostname() {
			if (expirationTimestamp < System.currentTimeMillis() && updateRunning.compareAndSet(false, true))
				updateCache();
		...
		public void updateCache() {
			...
			try {
				...
				logger.info(
						"Localhost hostname lookup failed, keeping the value if this persists it may mean your DNS is incorrectly configured and you may want to hardcode your server name: https://docs.sentry.io/clients/java/config/"
								+ hostname + ' ' + e);
			}
		}
	}
	 ...
	 *
	 * @param string
	 *            string from which a checksum should be obtained
	 * @return a checksum allowing two events with the same properties to be grouped
	 *         later.
	 */
	 ...
	 *
	 * @param eventId
	 *            unique identifier for the new event.
	 */
	...
	private void autoSetMissingValues() {
		...
		if (event.getSdk() == null)
			event.setSdk(new Sdk(SentryEnvironment.SDK_NAME, SentryEnvironment.SDK_VERSION, sdkIntegrations));
	 ...
	 * <p>
	 * This operations will automatically set the missing values and make the
	...
	public synchronized Event build() {
		...
		// Lock it only when everything has been set, in case of exception it should be
		// possible to try to build again.
		alreadyBuilt = true;
		return event;
	}
	...
	/**
	 * Ensures that every field in the {@code Event} are immutable to avoid
	 * confusion later.
	 */
	private void makeImmutable() {
		...
		// Make the extra properties unmodifiable (everything in it is still mutable
		// though)
	...
	public String toString() {
		return "EventBuilder{" + "event=" + event + ", alreadyBuilt=" + alreadyBuilt + '}';
	}
	 ...
	 *
	 * @param frame
	 *            stack frame during which the event was captured.
	 * @return the current {@code EventBuilder} for chained calls.
	...
	public EventBuilder withCulprit(SentryStackTraceElement frame) {
		return withCulprit(
				buildCulpritString(frame.getModule(), frame.getFunction(), frame.getFileName(), frame.getLineno()));
	}
	...
	public EventBuilder withCulprit(StackTraceElement frame) {
		return withCulprit(buildCulpritString(frame.getClassName(), frame.getMethodName(), frame.getFileName(),
				frame.getLineNumber()));
	}
	 ...
	 *
	 * @param dist
	 *            application distribution version in the event.
	 * @return the current {@code EventBuilder} for chained calls.
	 */
	 ...
	 *
	 * @param environment
	 *            application environment in the event.
	 * @return the current {@code EventBuilder} for chained calls.
	 */
	...
	/**
	 * Sets event fingerprint, a list of strings used to dictate the deduplicating
	 * for this event.
	 *
	 * @param fingerprint
	 *            fingerprint
	...
	/**
	 * Sets event fingerprint, an array of strings used to dictate the deduplicating
	 * for this event.
	 *
	 * @param fingerprint
	 *            fingerprint
	 ...
	 *
	 * @param release
	 *            application release version in the event.
	 * @return the current {@code EventBuilder} for chained calls.
	 */
	 ...
	 * <p>
	 * If a {@code SentryInterface} with the same interface name has already been
	 * added, the new one will replace the old one.
	 *
	 * @param sentryInterface
	 *            sentry interface to add to the event.
	 * @return the current {@code EventBuilder} for chained calls.
	 */
	 ...
	 * Adds a {@link SentryInterface} to the event.
	 * <p>
	 * Checks whether or not the entry already exists, and replaces it only if
	 * {@code replace} is true.
	 *
	 * @param sentryInterface
	 *            sentry interface to add to the event.
	 * @param replace
	 *            If true and a Sentry Interface with the same name has already been
	 *            added it will be replaced. If false the statement will be ignored.
	 * @return the current {@code EventBuilder} for chained calls.
	 */
	 ...
	 *
	 * @param serverName
	 *            name of the server responsible for the event.
	 * @return the current {@code EventBuilder} for chained calls.
	 */
