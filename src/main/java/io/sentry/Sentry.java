...
/**
 * Sentry provides easy access to a statically stored {@link SentryClient}
 * instance.
 */
public final class Sentry {
	private static final Logger logger = Logger.getLogger(Sentry.class.getName());
	/**
	 * The most recently constructed {@link SentryClient} instance, used by static
	 * helper methods like {@link Sentry#capture(Event)}.
	 */
	private static volatile SentryClient storedClient = null;
	/**
	 * Tracks whether the {@link #init()} method has already been attempted
	 * automatically by {@link #getStoredClient()}.
	 */
	 ...
	 *
	 * @param event
	 *            Event to send to the Sentry server.
	 */
	...
	/**
	 * Builds and sends an {@link Event} to the Sentry server using the statically
	...
	/**
	 * Sends an exception (or throwable) to the Sentry server using the statically
	 * stored {@link SentryClient} instance.
	 * <p>
	...
	/**
	 * Close the stored {@link SentryClient}'s connections and remove it from static
	 * storage.
	 */
	...
	/**
	 * Returns the last statically stored {@link SentryClient} instance. If no
	 * instance is already stored, the {@link #init()} method will be called one
	 * time in an attempt to create a {@link SentryClient}.
	 *
	...
	/**
	 * Initialize and statically store a {@link SentryClient} by looking up a
	...
	/**
	 * Initialize and statically store a {@link SentryClient} by looking up a
	 * {@link Dsn} and using the provided {@link SentryClientFactory}.
	 *
	 * @param sentryClientFactory
	 *            SentryClientFactory to use.
	 * @return SentryClient
	 */
	 ...
	 * <p>
	 * Note that the Dsn or SentryClientFactory may be null, at which a best effort
	 * attempt is made to look up or choose the best value(s).
	 *
	 * @param dsn
	 *            Data Source Name of the Sentry server.
	 * @param sentryClientFactory
	 ...
	 *
	 * @param breadcrumb
	 *            Breadcrumb to record.
	 * @deprecated use {@link Sentry#getContext()} and then
	 *             {@link Context#recordBreadcrumb(Breadcrumb)}.
	 */
	@Deprecated
	 ...
	 *
	 * @param client
	 *            {@link SentryClient} instance to store.
	 */
	public static void setStoredClient(SentryClient client) {
		if (storedClient != null)
			logger.fine(String.format("Overwriting statically stored SentryClient instance {} with {}.", storedClient,
					client));
		storedClient = client;
	}
