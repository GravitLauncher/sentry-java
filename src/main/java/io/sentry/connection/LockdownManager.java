...
public class LockdownManager {
	 ...
	 * <p>
	 * On each attempt the time is doubled until it reaches
	 * {@link #maxLockdownTime}.
	 */
	private long baseLockdownTime = DEFAULT_BASE_LOCKDOWN_TIME;
	/**
	 * Number of milliseconds after lockdownStartTime to lockdown for, or 0 if not
	 * currently locked down.
	 */
	private long lockdownTime = 0;
	/**
	 * Timestamp of when the current lockdown started, or null if not currently
	 * locked down.
	 */
	 ...
	 *
	 * @param clock
	 *            Clock object to use for lockdown logic
	 */
	...
	/**
	 * Enable lockdown if it's not already enabled, using the recommended time from
	 * the provided {@link ConnectionException}, if any.
	 *
	 * @param connectionException
	 *            ConnectionException to check for a recommended lockdown time, may
	 *            be null
	...
	/**
	 * Reset the lockdown state, disabling lockdown and setting the backoff time to
	 * zero.
	 */
