 ...
 * <p>
 * It is possible to enable the "naive mode" to allow a connection over SSL
 * using a certificate with a wildcard.
 */
public class HttpConnection extends AbstractConnection {
	/**
	 * HTTP code `429 Too Many Requests`, which is not included in
	 * HttpURLConnection.
	 */
	...
	/**
	 * HostnameVerifier allowing wildcard certificates to work without adding them
	 * to the truststore.
	 */
	private static final HostnameVerifier NAIVE_VERIFIER = (hostname, sslSession) -> true;

	/**
	 * Automatically determines the URL to the HTTP API of Sentry.
	 *
	 * @param sentryUri
	 *            URI of the Sentry server.
	 * @param projectId
	...
	/**
	 * Setting allowing to bypass the security system which requires wildcard
	 * certificates to be added to the truststore.
	 */
	 ...
	 * @param publicKey
	 *            public key of the current project.
	 * @param secretKey
	 *            private key of the current project.
	 * @param proxy
	 *            address of HTTP proxy or null if using direct connections.
	 * @param eventSampler
	 *            EventSampler instance to use, or null to not sample events.
	 */
		...
		try {
			...
			try {
				...
				if (responseCode == HttpURLConnection.HTTP_FORBIDDEN) {
					...
					/*
					 * If the response is a 429 we rethrow as a TooManyRequestsException so that we
					 * can avoid logging this is an error.
					 */
	...
	/**
	 * This will set the timeout that is used in establishing a connection to the
	 * url. By default this is set to 5 second.
	 *
	 * @param timeout
	 *            New timeout to set. If 0 is used (java default) wait forever.
	 */
	...
	/**
	 * This will set the timeout that is used in reading data on an already
	 * established connection. By default this is set to 1 seconds.
	 *
	 * @param timeout
	 *            New timeout to set. If 0 is used (java default) wait forever.
	 */
	...
	/**
	 * This will set the timeout that is used in establishing a connection to the
	 * url. By default this is set to 1 second.
	 *
