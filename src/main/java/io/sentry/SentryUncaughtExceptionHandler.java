...
/**
 * Sends any uncaught exception to Sentry, then passes the exception on to the
 * pre-existing uncaught exception handler.
 */
public class SentryUncaughtExceptionHandler implements Thread.UncaughtExceptionHandler {
	...
	/**
	 * Configures an uncaught exception handler which sends events to Sentry, then
	 * calls the preexisting uncaught exception handler.
	 *
	 ...
	 * handler (and is therefore not the
	 * {@link Thread#getDefaultUncaughtExceptionHandler()}), this boolean is the
	 * only way we can disable it.
	 */
	private volatile Boolean enabled = true;

	/**
	 * Construct the {@link SentryUncaughtExceptionHandler}, storing the
	 * pre-existing uncaught exception handler.
	 *
	...
	public void disable() {
		...
		// It's possible that another uncaught exception handler was installed 'over'
		// us.
		// Whether or not it wrapped us, we have no control over what other classes do,
		// and
		// so we can only remove ourselves if we are still the ('top most') default
		// handler.
		// The 'enabled' boolean exists to ensure we no longer handle uncaught
		// exceptions
	...
	public void uncaughtException(Thread thread, Throwable thrown) {
		if (enabled) {
			...
			EventBuilder eventBuilder = new EventBuilder().withMessage(thrown.getMessage()).withLevel(Event.Level.FATAL)
