 ...
 * {@link SentryClientFactory#createSentryClient(io.sentry.dsn.Dsn)}, which will
 * use the best factory available.
 */
public class SentryClient {
	 ...
	 * <p>
	 * Might be null in which case the release information will not be sent with the
	 * event.
	 */
	 ...
	 * <p>
	 * Might be null in which case the release distribution will not be sent with
	 * the event.
	 */
	 ...
	 * <p>
	 * Might be null in which case the environment information will not be sent with
	 * the event.
	 */
	...
	/**
	 * Set of callbacks that are checked before each {@link Event} is sent to
	 * Sentry.
	 */
	...
	/**
	 * The {@link ContextManager} to use for locating and storing data that is
	 * context specific, such as {@link io.sentry.event.Breadcrumb}s.
	 */
	private final ContextManager contextManager;
	/**
	 * Reference to the {@link SentryUncaughtExceptionHandler} if one was enabled,
	 * or null.
	 */
	 ...
	 *
	 * Note that the most recently constructed instance is stored statically so it
	 * can be used with the static helper methods.
	 *
	 * @param connection
	 *            Underlying {@link Connection} instance to use for sending events
	 * @param contextManager
	 *            {@link ContextManager} instance to use for storing contextual data
	 */
	 ...
	 *
	 * @param extraName
	 *            Tag name to extract from the MDC system
	...
	/**
	 * Runs the {@link EventBuilderHelper} against the {@link EventBuilder} to
	 * obtain additional information with a MDC-like system.
	 *
	 * @param eventBuilder
	 *            event builder containing a not yet finished event.
	 */
	 ...
	 *
	 * @param eventBuilder
	 *            {@link EventBuilder} to send to Sentry.
	 */
	 ...
	 *
	 * @param throwable
	 *            exception to send to Sentry.
	 */
	public void sendException(Throwable throwable) {
		EventBuilder eventBuilder = new EventBuilder().withMessage(throwable.getMessage()).withLevel(Event.Level.ERROR)
	 ...
	 *
	 * @param extraTags
	 *            Set of tags to extract from the MDC system
	...
	public String toString() {
		return "SentryClient{" + "release='" + release + '\'' + ", dist='" + dist + '\'' + ", environment='"
				+ environment + '\'' + ", serverName='" + serverName + '\'' + ", tags=" + tags + ", mdcTags=" + mdcTags
				+ ", extra=" + extra + ", connection=" + connection + ", builderHelpers=" + builderHelpers
				+ ", contextManager=" + contextManager + ", uncaughtExceptionHandler=" + uncaughtExceptionHandler + '}';
	}
}
